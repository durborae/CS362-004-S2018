/* 
    ...
    Course: CS 362
    Assignment: Assignment 4, Random Great Hall Card Testing
    Date: 05/12/2018
*/
	...
	for (testCounter = 0; testCounter < numTests; testCounter++) {
		...
		for (counter = 0; counter < numPlayers; counter++) {
			...
			if (counter == randomPlayer) {
				...
				if ((test.numActions) == (actual.numActions + actionsAdded)) {
					test_passed();
				}
				else if (test.numActions != (actual.numActions + actionsAdded)) {
					test_failed();

					//printf("\t\tExpected Number of Actions: %d\n", (actual.numActions + actionsAdded));
